...
- Per-case MSW handler: `s1-summary-list-detail.ts`
  ...
  - `GET */summary/api/v1/summaries/9101` — 返回 `{code,message,data: SummaryDetail}`，包含 `result.abstract`、`result.content`、`sources` 和 creator 权限字段。
...
- `packages/dmworksummary/src/module.tsx:159`: 顶层菜单 id 为 `summary`，菜单文案来自 `summary.menu.title`。
