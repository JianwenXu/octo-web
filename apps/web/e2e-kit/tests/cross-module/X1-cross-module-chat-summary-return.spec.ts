...
test("@X1 @p1 @cross-module @chat @summary @deep-link Chat 进入 Summary 详情后返回当前会话", async ({ authedPage }) => {
  ...
  await installMockImRuntime(authedPage, {
    ...
    groups: [{ group_no: "s22-project-group", name: "X1 项目群" }],
    conversations: [{ channelId: "s22-project-group", channelType: 2, unread: 0 }],
    messages: [{ channelId: "s22-project-group", channelType: 2, messageSeq: 1, fromUid: "e2e-user-1", content: { type: 1, text: "X1 项目群历史消息" } }],
  ...
  await expect(authedPage.getByText("X1 项目群", { exact: true })).toBeVisible({ timeout: 15_000 });
  await authedPage.getByText("X1 项目群", { exact: true }).click();
  await expect(authedPage.locator(".wk-chat-conversation-header-channel-info-name", { hasText: "X1 项目群" })).toBeVisible({ timeout: 15_000 });
  ...
  await expect(panel.getByText("S22 聊天内总结", { exact: true })).toBeVisible();
  await expect(authedPage.getByRole("heading", { name: "X1 项目群", level: 3 })).toBeVisible();
});
