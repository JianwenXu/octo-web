...
export async function registerGS3GlobalSearchFileFilter(page: Page): Promise<void> {
  await page.evaluate(() => {
    ...
    msw.worker.use(
      ...
      msw.http.post("*/_search_global_files", () =>
        msw.HttpResponse.json({
          data: [{ message_id: "gs3-file", message_seq: 2, file_name: "GS3 文件.pdf", file_size_bytes: 1024, file_ext: "pdf", download_url: "https://example.test/gs3-file.pdf", sender_id: "e2e-user-1", sender_name: "E2E Tester", sent_at: "2026-08-25T08:00:00Z", channel_id: "gs3-group", channel_type: 2 }],
