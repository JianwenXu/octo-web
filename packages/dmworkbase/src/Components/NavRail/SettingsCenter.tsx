...
export default function SettingsCenter({ visible, isDesktop = false, environment, hasAccountCenter = false, onClose, onSpaceManagement, onLogout, onSecrets, onVoice, onAbout }: SettingsCenterProps) {
  ...
  return <WKModal visible={visible} onCancel={onClose} title={t("base.navRail.settingsCenter.title")} width="min(1080px, calc(100vw - 48px))" className="wk-settings-center-modal" bodyStyle={{ padding: 0 }} options={{ maskClosable: true }}><div className="wk-settings-center" data-testid="settings-center"><aside className="wk-settings-center__sidebar" aria-label={t("base.navRail.settingsCenter.navigation")}><h1>{t("base.navRail.settingsCenter.title")}</h1><nav className="wk-settings-center__navigation">{availableGroups.map((group) => <section className="wk-settings-center__group" key={group.titleKey}><h2>{t(group.titleKey)}</h2><div className="wk-settings-center__nav-list">{group.items.map((item) => <button type="button" key={item.id} data-testid={`settings-center-nav-${item.id}`} className={`wk-settings-center__nav-item${selected?.id === selectedId ? " is-active" : ""}`} aria-current={selected?.id === selectedId ? "page" : undefined} onClick={() => setSelectedId(item.id)}><SettingsIcon name={item.id} /><span>{t(item.labelKey)}</span></button>)}</div></section>)}</nav><div className="wk-settings-center__footer">{onSpaceManagement && <button type="button" data-testid="settings-center-space-management" onClick={onSpaceManagement}>⌘ {t("base.navRail.settingsPanel.spaceManagement")}</button>}{onLogout && <button type="button" className="is-danger" data-testid="settings-center-logout" onClick={onLogout}>↪ {t("base.navRail.settingsPanel.logout")}</button>}</div></aside><main className="wk-settings-center__content" data-testid="settings-center-content"><button type="button" className="wk-settings-center__close" aria-label={t("base.common.close")} onClick={onClose}>×</button><SettingsPage item={selected} environment={runtimeEnvironment} onSecrets={onSecrets} onVoice={onVoice} onAbout={onAbout} /></main></div></WKModal>;
}
