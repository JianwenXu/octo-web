...
export default function QuickMuteSidebar({ service = defaultService }: { service?: QuickMuteService }) {
  ...
  return <div className="wk-navrail__quick-mute-wrap">
    ...
    <NavFlyout open={open} triggerRef={triggerRef} onOpenChange={setOpen} size="md" role="menu" ariaLabel={t("base.navRail.quickMute.menuTitle")} className="wk-navrail__quick-mute-menu">
      ...
      <button type="button" role="menuitem" disabled={busy} className="wk-navrail__quick-mute-option" onClick={() => void apply("manual")}>{t("base.navRail.settingsCenter.action.muteManual")}</button>
