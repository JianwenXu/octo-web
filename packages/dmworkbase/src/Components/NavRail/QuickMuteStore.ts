...
export class QuickMuteStore implements QuickMuteService {
  ...
  private state: QuickMuteState = { active: false, scope: "sound-and-popup", revision: 0 };
  ...
  private apply(next: QuickMuteState) {
    ...
    this.state = { ...next, scope: "sound-and-popup" };
  ...
  reset() {
    ...
    this.state = { active: false, scope: "sound-and-popup", revision: 0 };
    this.listeners.forEach((listener) => listener(this.state));
  }
